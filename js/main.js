...
function checkExpiration(){
  ...
  if(!firstRun){ firstRun=Date.now(); localStorage.setItem(EXP_KEY, firstRun);}
  const now=Date.now();
  const diffDays=(now-firstRun)/(1000*60*60*24);
  const remaining=Math.max(0, Math.ceil(EXPIRATION_DAYS-diffDays));
  document.getElementById("notice").innerText=remaining>0?`⚠️ Versão FREE: ${remaining} dia(s) restante(s)`:"🚫 Expirado!";
  if(remaining<=0){ showAlert("O período de uso gratuito expirou. Atualize para a versão PRO."); return 0; }
  return remaining;
}
